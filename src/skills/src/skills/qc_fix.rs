...
Return null for filters that don't apply. If the issue requires only audio or only video filters, leave the other as null."#,
        ...
        let diagnosis = fix_plan["diagnosis"].as_str().unwrap_or("Unknown issue");
        let video_filter = fix_plan["filters"]["video_filter"].as_str();
        let audio_filter = fix_plan["filters"]["audio_filter"].as_str();
        let confidence = fix_plan["confidence"].as_f64().unwrap_or(0.0);
        let notes = fix_plan["notes"].as_str().unwrap_or("");
        ...
        let summary = format!(
            "QC Fix applied: {}\n\
             ...
             Output: {}",
            diagnosis,
            video_filter.unwrap_or("none"),
            audio_filter.unwrap_or("none"),
            confidence * 100.0,
            notes,
            output_path
        );
        ...
        Ok(SkillResult {
            ...
            timecoded_findings: Some(vec![Finding {
                timecode_seconds: start_time.unwrap_or(0.0),
            ...
            action_taken: Some(format!("Applied ffmpeg filters: vf={}, af={}", 
                video_filter.unwrap_or("none"), audio_filter.unwrap_or("none"))),
